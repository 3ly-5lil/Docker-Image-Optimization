...
image info: ![V1 image info](../assets/v1.png)
...
image info: ![V2 image info](../assets/v2.png)
...
![V1 with .dockerignore](../assets/v1-dockerignore.png)
![V2 with .dockerignore](../assets/v2-dockerignore.png)
...
![V3 image info](../assets/v3.png)

> **Review Note:** Wait! If you delete `node_modules` inside the same layer after building the app, running `npm run preview` in your `CMD` will fail. The `preview` command relies on `node_modules` (specifically the Vite CLI) to spin up the local preview server. This is exactly why **Multi-Stage Builds** (../assets/v4) are the correct solution: they let you leave the source and `node_modules` behind, purely copying the compiled static assets into an `nginx` image to serve them.
...
![V4 image info](../assets/v4.png)
![V4.1 image info](../assets/v4.1.png)
